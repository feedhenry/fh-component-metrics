...
StatsdClient.prototype.buildMessages = function(data) {
  ...
  var tagsField = data.tags || {};
  //to make sure the data will show up in the studio, we need to add the `appname` and `statsType` as part of the tags so that it can be used as part of the filter.
  var appname = process.env.FH_APPNAME || 'NO-APPNAME-DEFINED';
  var statsType = 'app'; //either `api` or `app`. `api` for stats generated by the fh-mbaas-api
  tagsField.id = [appname, statsType].join('_');
  var tags = _.map(tagsField, function(tagValue, tagKey) {
